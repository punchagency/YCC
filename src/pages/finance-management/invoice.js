...
const Invoice = () => {
  ...
  const applyFilters = useCallback(() => {
    ...
    if (selectedName) {
      filteredData = filteredData.filter(
        (vendor) => vendor.name === selectedName
      );
    }

    if (selectedDate) {
      filteredData = filteredData.filter(
        (vendor) => vendor.date === selectedDate
      );
    }
    if (selectedStatus) {
      filteredData = filteredData.filter(
        (vendor) => vendor.status === selectedStatus
      );
    }
  ...
  const dates = [...new Set(vendors.map((vendor) => vendor.date))].map(
    (date) => ({ name: date, value: date })
  );

  const statuses = [...new Set(vendors.map((vendor) => vendor.status))].map(
    ...
    <>
      ...
      <div className="card-wrapper-gap">
        <DataTable
          ...
          onRowClick={(e) =>
            navigate(`/finance-management/invoice/${e.data.id}`)
          }
