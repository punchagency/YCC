...
const Expense = () => {
    ...
    setTimeout(() => {
      const fetchedVessels = [
        ...
        },
        {
          id: 3,
          name: "Aquaholic",
          expenseCategory: "Insurance Premiums",
          amount: "$400",
          date: "26/10/2024",
          vendor: "Mazagon",
          paymentMethod: "Cash",
  ...
  const applyFilters = useCallback(() => {
    ...
    if (selectedName) {
      filteredData = filteredData.filter(
        (expense) => expense.name === selectedName
      );
    }
    if (selectedCategory) {
      filteredData = filteredData.filter(
  ...
  // Dropdown options
  const vesselNames = [...new Set(expenses.map((expense) => expense.name))].map(
  ...
  const statuses = [...new Set(expenses.map((expense) => expense.status))].map(
    (status) => ({ name: status, value: status })
  );

  const date = [...new Set(expenses.map((expense) => expense.date))].map(
    ...
    <>
      ...
      >
        <Button
          ...
          onClick={(e) => navigate(`/finance-management/expense/edit`)}
        />
        <Button
          label="Update"
    ...
    <>
      ...
      <div className="card-wrapper-gap">
        <DataTable
          ...
          onRowClick={(e) =>
            navigate(`/finance-management/expense/${e.data.id}`)
          }
          ...
          <Column
            ...
            body={(rowData) => (
              <span
                style={{
                  backgroundColor:
                    rowData.status === "Paid" ? "#94E0ED" : "#EF4444",
