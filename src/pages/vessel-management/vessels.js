...
const Vessels = () => {
    ...
    setTimeout(() => {
      const fetchedVessels = [
        ...
        },
        {
          id: 6,
          name: "Ocean Pearl",
          number: "HODCA 1234 AB",
          type: "Luxury Yacht",
          manufacturer: "Westport Yachts",
          yearbuilt: "2023",
          status: "Inactive",
        },
        {
  ...
  const applyFilters = useCallback(() => {
    ...
    if (selectedType) {
      filteredData = filteredData.filter(
        (vessel) => vessel.type === selectedType
      );
    }
    if (selectedYear) {
      filteredData = filteredData.filter(
        (vessel) => vessel.yearbuilt === selectedYear
      );
    }
    if (selectedStatus) {
      filteredData = filteredData.filter(
  ...
  // Dropdown options
  const vesselTypes = [...new Set(vessels.map((vessel) => vessel.type))].map(
    (type) => ({ name: type, value: type })
  );

  const years = [...new Set(vessels.map((vessel) => vessel.yearbuilt))].map(
    (year) => ({ name: year, value: year })
  );

  const statuses = [...new Set(vessels.map((vessel) => vessel.status))].map(
    ...
    <>
      ...
      <div className="card-wrapper-gap">
        <DataTable
          ...
          onRowClick={(e) =>
            navigate(`/vessel-management/vessels/${e.data.id}`)
          }
          ...
          <Column
            ...
            body={(rowData) => (
              <span
                style={{
                  backgroundColor:
                    rowData.status === "Active" ? "#CAF1D8" : "#EF4444",
